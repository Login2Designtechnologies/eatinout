...
import 'package:dineout/Getx_Controller/Wallet_controller.dart';
import 'package:dineout/Payment/Bill_paid.dart';
import 'package:dineout/Payment/web_view.dart';
...
import 'package:dineout/paymentscreen/PayStack.dart';
...
class _PaymentDetailsState extends State<PaymentDetails> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    paymentgatewayController.paymentgatewaySuccessEmail();
    paymentgatewayController.paymentHistory(
      discountamount: billdiscount,
      discountvalue: widget.tip.toString(),
      payedamount: double.parse(totelbill).toStringAsFixed(2).toString(),
      paymentid: selectidPay,
      restid: widget.restid.toString(),
      tipamount: widget.tip.toString(),
                  ...
                  child: GestButton(
                    ...
                    onclick: () {
                      ...
                      if (paymenttital == "Razorpay") {
                        ...
                        paystackController.getPaystack(amount: totelbill).then(
                          (value) {
                            Get.to(() => PaymentWebVIew(
                        ...
                        Get.to(() => PaymentWebVIew(
                                  initialUrl:
                                      "${AppUrl.paymentBaseUrl}Payfast/index.php?amt=${totelbill}",
                                  navigationDelegate:
                                      (NavigationRequest request) async {
                                    print(
                                        "=========== initial ${AppUrl.paymentBaseUrl}Payfast/index.php?amt=${totelbill}");
                                    final uri = Uri.parse(request.url);
                        ...
                        webViewWalletAddAmount(
                          initialUrl:
                              "${AppUrl.paymentBaseUrl}Midtrans/index.php?name=test&email=${getData.read("UserLogin")["email"]}&phone=${getData.read("UserLogin")["ccode"] + getData.read("UserLogin")["mobile"]}&amt=${totelbill}",
                          status1: "status_code",
                          status2: "200",
                        );
                        ...
                        Get.to(() => PaymentWebVIew(
                              initialUrl:
                                  "${AppUrl.paymentBaseUrl}Khalti/index.php?amt=${totelbill}",
                              navigationDelegate:
                                  (NavigationRequest request) async {
                                print(
                                    "=========== initial ${AppUrl.paymentBaseUrl}Khalti/index.php?amt=${totelbill}");
                                final uri = Uri.parse(request.url);
                                ...
                                if (uri.queryParameters["status"] == null) {
                                  ...
                                  if (uri.queryParameters["status"] ==
                                    ...
                                    Get.back();
                                    showToastMessage(
                                        "${uri.queryParameters["ResponseMsg"]}");
                                  }
                                }
                        ...
                        Get.to(() => PaymentWebVIew(
                                  initialUrl:
                                      "${AppUrl.paymentBaseUrl}2checkout/index.php?amt=${totelbill}",
                                  navigationDelegate:
                                      (NavigationRequest request) async {
                                    final uri = Uri.parse(request.url);
                      ...
                      //========================================= done.
                        ...
                        Get.to(() => PaymentWebVIew(
                                  initialUrl:
                                      "${AppUrl.paymentBaseUrl}merpago/index.php?amt=${totelbill}",
                                  navigationDelegate:
                                      (NavigationRequest request) async {
                                    final uri = Uri.parse(request.url);
                                    print("URL + ${uri.queryParameters}");
